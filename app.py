...
# ---------------- Sidebar controls ----------------
st.sidebar.markdown('<div class="sidebar-section"><h3 style="margin:0; color:#e4e7eb;">⚙️ Configuration</h3></div>', unsafe_allow_html=True)
...
with col4:
    primary_symbol = st.selectbox("Primary", options=symbols if symbols else ["btcusdt"])

autorefresh = st.sidebar.checkbox("🔄 Auto-refresh (30s)", value=True)

st.sidebar.markdown('<div class="sidebar-section"><h3 style="margin:0; color:#e4e7eb;">📡 Data Ingestion</h3></div>', unsafe_allow_html=True)
...
st.sidebar.markdown('<div class="sidebar-section"><h3 style="margin:0; color:#e4e7eb;">📤 Data Management</h3></div>', unsafe_allow_html=True)
...
if uploaded is not None:
    try:
        ...
        if not required.issubset(set(df_upload.columns)):
            ...
            df_upload['ts_parsed'] = pd.to_datetime(df_upload['ts'], utc=True, infer_datetime_format=True, errors='coerce')
            df_good = df_upload.dropna(subset=['ts_parsed', 'open', 'high', 'low', 'close'])
...
if len(symbols) >= 2:
    try:
        pair_res = analytics.compute_pair_analytics(symbols[0], symbols[1], timeframe=timeframe, rolling=rolling_window)
    except Exception:
        pair_res = None
...
if pair_res:
    try:
        ...
        last_hedge = float(pair_res.get("hedge")) if pair_res.get("hedge") is not None else None
...
st.markdown('<p style="color:#94a3b8; font-size:1.1rem; margin-bottom:2rem;">Real-time analytics for statistical arbitrage and market microstructure</p>', unsafe_allow_html=True)
...
with tab1:
    ...
    if df_ohlc.empty:
        ...
        if primary_symbol in df_ohlc['symbol'].unique():
            ...
            st.plotly_chart(fig, use_container_width=True, key="market_candles_a")
        else:
...
with tab2:
    ...
    if len(symbols) < 2:
        ...
        st.plotly_chart(fig, use_container_width=True, key="pair_spread_zscore_a")
